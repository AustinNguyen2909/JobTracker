import { Calendar, Clock, Link as LinkIcon, CheckCircle2, Circle } from 'lucide-react';
...
export default function TaskCard({ task, onClick, onToggleComplete }: TaskCardProps) {
  ...
  const isOverdue = deadlineDate && deadlineDate < new Date() && !task.isCompleted;
  ...
  return (
    ...
    >
      ...
      <div className="task-card-body">
        {/* Toggle */}
        <button
          onClick={(e: MouseEvent<HTMLButtonElement>) => { e.stopPropagation(); onToggleComplete(task.id); }}
          className="task-toggle"
          style={{ color: task.isCompleted ? 'var(--success)' : 'var(--text-muted)' }}
        >
