...
import type { Task, TaskFormData, TabId, FilterId, PriorityFilterId } from './types';
import './App.css';
...
function AppContent() {
  ...
  const openCreate = () => { setModalTask(null); setShowModal(true); };
  ...
  const filteredTasks = useMemo(() => {
    ...
    return tasks.filter(task => {
      ...
      if (activeFilter === 'active' && task.isCompleted) return false;
      if (activeFilter === 'completed' && !task.isCompleted) return false;
      if (activeFilter === 'upcoming' && (!task.deadline || new Date(task.deadline) <= now || task.isCompleted)) return false;
      if (activeFilter === 'past' && (!task.deadline || new Date(task.deadline) > now || task.isCompleted)) return false;
    ...
    return (
      <div className="app-shell">
        <div className="loading-center">
          ...
          <button onClick={() => window.location.reload()} className="btn-create" style={{ marginTop: 12 }}>
            Retry
          </button>
        </div>
      </div>
